...
class PanelMaterial: UIViewController, Panelable {
   ...
   var ref: DatabaseReference!
    ...
    @IBOutlet weak var CoolDownTimerLbl: UILabel!
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector:#selector(self.AwakeSleep(notification:)),name: UIApplication.didBecomeActiveNotification,object: nil)
            ...
            self.CoolDownTimerLbl.isHidden = true
      
        }
        
        headerTitle.sizeToFit()
        self.headerTitle.text = "Select an ATM"
...
//MARK: - Timer and Countdown
    ...
    @objc func updateTimer() {
                ...
                self.CoolDownTimerLbl.text = self.timeFormatted(self.CoolDownTime) // will show timer
                if CoolDownTime != 0 {
                    CoolDownTime -= 1  // decrease counter timer
...
//MARK: - Voting System
    @IBAction func VoteUp(_ sender: UIButton) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3) {
            self.ThumbsUpBtn.isEnabled = false
            self.ThumbsDownbtn.isEnabled = false
            self.CoolDownTimerLbl.isHidden = false
            self.startOtpTimer()
        }
        UpdateVotingLabels()
    ...
    func enableButton() {
        ThumbsUpBtn.isEnabled = true
        ThumbsDownbtn.isEnabled = true
        CoolDownTimerLbl.isHidden = true
    }
    ...
    @IBAction func VotesDown(_ sender: UIButton) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
            self.ThumbsDownbtn.isEnabled = false
            self.LblThumbsUpCount.isEnabled = false
            self.CoolDownTimerLbl.isHidden = false
            self.startOtpTimer()
        }
